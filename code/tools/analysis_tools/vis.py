...
def parse_args():
    ...
    parser.add_argument('--root_path',
                        type=str,
                        default='/root/gekim/distill-bev/data/nuscenes',
                        help='Path to nuScenes dataset')
    parser.add_argument('--save_path',
                        type=str,
                        default='./vis_main_result/vis_bevdepth_with_gt3',
...
def main():
    ...
    for cnt, infos in enumerate(dataset['infos'][1500:2000]):
        if cnt % 10 == 0:
        ...
        for view in views:
            ...
            old_prefix = "/raid/datasets/NuScenes/nuscenes/"
            new_prefix = "/root/gekim/distill-bev/data/nuscenes/"

            # 기존 경로에서 새로운 경로로 변환
            infos['cams'][view]['data_path'] = infos['cams'][view]['data_path'].replace(old_prefix, new_prefix)
            infos['lidar_path'] = infos['lidar_path'].replace(old_prefix, new_prefix)
        ...
        img = np.zeros((900 * 2, 1600 * 3 + canva_size * 2, 3),
                       dtype=np.uint8)
        img[:900, :1600*3, :] = np.concatenate(imgs[:3], axis=1)
        img[900:, :1600*3, :] = np.concatenate(imgs[3:], axis=1)
        img[:, 1600*3:, :] = cv2.resize(canvas, (canvas.shape[0]*2, canvas.shape[1]*2))
