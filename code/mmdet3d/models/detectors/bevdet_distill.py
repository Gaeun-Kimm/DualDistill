...
from .scaleKD_attention import WindowMultiheadPosAttention
...
class BEVDetDistill(BEVDet):
    def __init__(self, teacher_config, teacher_ckpt, distill_type, distill_params, eval_teacher=True, self_ckpt=None,
        ...
        if distill_type == 'fgd':
            ...
            distill_params['adaptation_type'] = ['CAP_orthogonal', 'CAP_orthogonal', '1x1conv']
            for index, (adaptation_type, teacher_adaptation_type, student_channel, teacher_channel) in \
                ...
                elif adaptation_type == 'cap':
                    if index == 0:
                        self.channel_wise_adaptations.append(CAP(student_dims=student_channel,
                                                                teacher_dims=teacher_channel,
                                                                query_hw=(128, 128),
                                                                pos_hw=(32, 32),      
                                                                pos_dims=64,        
                                                                window_shapes=(1, 1),  
                        ...
                        self.channel_wise_adaptations.append(CAP(student_dims=student_channel,
                                                                ...
                                                                self_query=True,
                                                                softmax_scale=1.0))
                elif adaptation_type == 'orthogonal':
                    self.channel_wise_adaptations.append(
                        nn.Sequential(
                            nn.Upsample(scale_factor=self.distill_params['student_adaptation_params']['upsample_factor'],
                                        mode='bilinear', align_corners=True),
                            OrthogonalLinear(in_dim=student_channel, out_dim=teacher_channel)))
                elif adaptation_type == 'CAP_orthogonal':
                    if index == 0:
                        self.channel_wise_adaptations.append(
                            nn.Sequential(
                                CAP(student_dims=student_channel,
                                    teacher_dims=student_channel,
                                    query_hw=(32, 32),
                                    pos_hw=(32, 32),      
                                    pos_dims=64,        
                                    window_shapes=(1, 1),  
    ...
    def foreground_scale_mask(self, student_H, student_W, gt_bboxes_3d, bg_extend_length=0, bg_extend_weight=0,):
        ...
        for boxes in gt_bboxes_3d:
            ...
            foreground_mask = mask.any(axis=-1).astype(float) #bbox에 포함되면 fg
        ...
        if self.distill_params['fp_as_foreground'][index] != 'none' and self._epoch >= self.distill_params['fp_epoch']:
            ...
            for b in range(B):
                ...
                if bg_points_number[b] > fp_points_number[b]:
                    bg_scale_mask[b][:] = 1.0 / (bg_points_number[b] - fp_points_number[b]) # 1/N_tn ?
                else:
                    bg_scale_mask[b][:] = 0
    ...
    def cap_distill_loss(self, teacher_feat, student_feat,
                         gt_bboxes_3d, gt_labels_3d,
                         canvas_feat,
                         heatmaps, teacher_preds, student_preds,
                         index):
    ...
    def distill_loss(self, teacher_feat, student_feat, teacher_preds, student_preds,
        ...
        #     tp_mask, fp_mask, fn_mask = self.s2m2_ssd_mask(heatmaps, student_preds)
            ...
            losses_distill = self.cap_distill_loss(teacher_feat, student_feat,
                                                   gt_bboxes_3d, gt_labels_3d,
                                                   canvas_feat,
                                                   heatmaps, teacher_preds, student_preds, index)
        else:
    ...
    def forward_distill(self, points, img_metas, gt_bboxes_3d, gt_labels_3d,
        ...
        #pseudo-label entropy minimization
        teacher_preds_for_crosskd = copy.deepcopy(teacher_preds)
        temperature = 0.1
        
        for task_id, preds_dict in enumerate(teacher_preds_for_crosskd):
            entropy_minimization_heatmap = preds_dict[0]['heatmap'] / temperature
            
            teacher_preds_for_crosskd[task_id][0]['heatmap'] = entropy_minimization_heatmap


        teacher_bbox_list = self.teacher_model.pts_bbox_head.get_bboxes(teacher_preds_for_crosskd, img_metas, rescale=False)
        teacher_bbox_results = [
            bbox3d2result(bboxes, scores, labels)
            for bboxes, scores, labels in teacher_bbox_list
        ]

        teacher_bboxes_3d = []
        teacher_labels_3d = []
        for i in range (len(teacher_bbox_results)):
            teacher_bboxes_3d.append(teacher_bbox_results[i]['boxes_3d'].to(device=teacher_neck_feat[0].device))
            teacher_labels_3d.append(teacher_bbox_results[i]['labels_3d'].to(device=teacher_neck_feat[0].device))
        
        cross_head_preds = self.teacher_model.pts_bbox_head([self.channel_wise_adaptations[2](img_feats[0])])

        crosskd_losses = self.teacher_model.pts_bbox_head.loss(teacher_bboxes_3d, teacher_labels_3d, cross_head_preds, get_targets=False)
        crosskd_losses = {f"crosskd.{key}": value for key, value in crosskd_losses.items()}
        ...
        new_losses_distill = {**new_losses_distill, **crosskd_losses}
...
class CAP(nn.Module):
    def __init__(self,
        ...
        self.pos_attention = WindowMultiheadPosAttention(teacher_dims, num_heads=num_heads, input_dims=student_dims, pos_dims=teacher_dims, window_shapes=window_shapes, softmax_scale=softmax_scale)
    ...
    def forward(self, x, query=None):
        ...
        fea_S = fea_S.permute(0, 2, 1).reshape(N, self.teacher_dims, H, W)
        return fea_S
