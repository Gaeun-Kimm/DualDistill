...
class CenterHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 crosskd_loss_cls=dict(type='GaussianFocalLoss', reduction='none'),
                 loss_bbox=dict(
        ...
        self.crosskd_loss_cls = build_loss(crosskd_loss_cls)
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, get_targets=False, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            # vis = (heatmaps[task_id].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
            # image = Image.fromarray(vis)
            # image.save("entropy_after.png")
    ...
    # @force_fp32(apply_to=('preds_dicts'))
    # def check_loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, get_targets=False, **kwargs):
    #     """Loss function for CenterHead.
    ...
    #         vis = (heatmaps[0][0].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_1.png")

    #         vis = (heatmaps[1][0][0].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_2_1.png")

    #         vis = (heatmaps[1][0][1].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_2_2.png")

    #         vis = (heatmaps[2][0][0].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_3_1.png")

    #         vis = (heatmaps[2][0][1].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_3_2.png")

    #         vis = (heatmaps[3][0].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_4.png")

    #         vis = (heatmaps[4][0][0].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_5_1.png")

    #         vis = (heatmaps[4][0][1].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_5_2.png")

    #         vis = (heatmaps[5][0][0].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_6_1.png")

    #         vis = (heatmaps[5][0][1].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
    #         image = Image.fromarray(vis)
    #         image.save("heatmap_vis/t1_6_2.png")
    ...
    @force_fp32(apply_to=('preds_dicts'))
    def crosskd_loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, total_scale_mask, get_targets=False, **kwargs):
        """Loss function for CenterHead.
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            loss_heatmap = self.crosskd_loss_cls(
                preds_dict[0]['heatmap'],
                heatmaps[task_id],
                avg_factor=max(num_pos, 1))
            ...
            avg_factor=max(num_pos, 1)
            loss_heatmap = (loss_heatmap * total_scale_mask).sum() / (avg_factor + eps)
            #loss_heatmap = (loss_heatmap * total_scale_mask).sum(dim=(1,2,3)).mean()
