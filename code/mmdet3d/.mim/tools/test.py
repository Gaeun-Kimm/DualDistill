...
def main():
    ...
    # so it fits into self.simple_test(points[0], img_metas[0], img[0], **kwargs) in forward_test in Base3DDetector
        ...
        cfg.model.teacher_config = 'configs/centerpoint/centerpoint_02pillar_second_secfpn_4x8_cyclic_20e_nus.py'
        cfg.data.test.pipeline = [
    ...
    if not args.save_attention:
        ...
        cfg.model.teacher_config='configs/dynamic_centerpoint/dynamic_centerpoint_02pillar_second_secfpn_4x8_cyclic_20e_nus.py'
        cfg.model.teacher_ckpt='ckpt/centerpoint_epoch_20.pth'
    ...
    if rank == 0:
        ...
        # if args.eval:
        #     eval_kwargs = cfg.get('evaluation', {}).copy()
        #     current_epoch = checkpoint.get('meta', {}).get('epoch', 0)  # 현재 에포크 가져오기
            
        #     # 평가 주기를 동적으로 설정
        #     if current_epoch < 20:
        #         interval = 4  # 20 에포크까지는 4 에포크마다
        #     else:
        #         interval = 1  # 20 에포크 이후부터는 1 에포크마다
        ...
        #     # 업데이트된 interval을 kwargs에 추가
        #     eval_kwargs.update(dict(metric=args.eval, interval=interval, **kwargs))
        #     print(dataset.evaluate(outputs, **eval_kwargs))
